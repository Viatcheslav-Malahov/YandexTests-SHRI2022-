export function generateLabyrinth(width, height, start, finish) {
    ...
    for (let cell = 0; cell < width; cell++) {
        if (cell !== 0) {
            data[cell] |= 32 /* Meta.Left */;
        }
        if (cell !== width - 1) {
            data[cell] |= 8 /* Meta.Right */;
        }
    }
    for (let line = 1; line < height; line++) {
        ...
        for (let cell = 0; cell < width; cell++) {
            ...
            if (isPicked && cell !== width - 1) {
                data[index] |= 8 /* Meta.Right */;
                data[index + 1] |= 32 /* Meta.Left */;
                pickedNum++;
            }
            else {
                const bottomIndex = index - Math.round(pickedNum * Math.random());
                const topIndex = bottomIndex - width;
                data[topIndex] |= 16 /* Meta.Bottom */;
                data[bottomIndex] |= 4 /* Meta.Top */;
                pickedNum = 0;
            }
        }
    }
    data[start.x + start.y * width] |= (1 /* Meta.Start */ | 64 /* Meta.Known */);
    data[finish.x + finish.y * width] |= 2 /* Meta.Finish */;
